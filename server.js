...
// Unit IV: MongoDB Setup (Local)
mongoose.connect('mongodb://127.0.0.1:27017/eblood_db')
    .then(() => console.log("✅ Database Connected"))
    .catch(err => console.error("❌ Connection Failed:", err));

// Schemas (Added Phone Number)
...
const appendWebsiteLog = (type, payload) => {
    ...
    fs.appendFile('donor_logs.txt', logEntry, () => {});
};
...
const validateDonorPayload = (payload) => {
    ...
    if (normalizedPhone.length !== 10) return { field: 'phone', message: 'Phone number should be exactly 10 digits.' };
...
// SEED ROUTE (Added Phone Numbers)
